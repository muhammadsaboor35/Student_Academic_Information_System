...
if (isset($_POST['signup'])) {
  try {
    ...
    if($isStudent) {
	$sql = "SELECT person_ID FROM Person NATURAL JOIN Admin
        	WHERE person_ID = :id
 	...
 	if (count($result) > 0) {
        	//login
		$sql = "INSERT INTO Teaching_Assistant values(:id)";
		$statement = $connection->prepare($sql);
		$statement->bindParam(':id', $id, PDO::PARAM_INT);
		$statement->execute();
   	}
        ...
        echo '<div class="alert alert-dismissible alert-danger">';
        echo ' <!--<button type="button" class="close" data-dismiss="alert">&times;</button>-->';
        echo 'Could not create Teaching Assistant account. You are not a Student yet. Please try again!.';
        echo '</div>';
    }
...
<html lang="en">
  ...
  <meta charset="utf-8" />
    ...
    <title>Signup | Teaching Assistant</title>
  ...
  <body>
	...
	<li class="nav-item pr-3">
	  <a class="nav-link active" href="Signup_as_Teaching_Assistant.php">Teaching Assistant</a>
	</li>
	<li class="nav-item pr-3">
	    ...
	    <div class="form-group custom-container2 container">
	      ...
	      <input type="password" class="form-control" name="password" placeholder="Enter password">
            ...
            <div>
              ...
              <a class="btn btn-link" href="Login_as_Teaching_Assistant.php" role="button">Already registered? Login</a>
            </div>
	  </fieldset>
	</form>
      </div>
