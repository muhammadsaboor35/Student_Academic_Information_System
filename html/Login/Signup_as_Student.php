...
if (isset($_POST['signup'])) {
  try {
    ...
    $sql = "SELECT instructor_ID from Instructor, Dept_Person WHERE instructor_ID = person_ID";
    $statement = $connection->prepare($sql);
    $statement->execute();
    $results = $statement->fetchAll();
    $advisor_ids = array();
    foreach($results as $item) {
	array_push($advisor_ids, $item[0]);
    }

    $advisor_id = $advisor_ids[array_rand($advisor_ids)];

    $sql = "INSERT INTO Student
	 	values (:id, :advisor_id, :curr_sem, :total_sem, :rank, :degree, :register_limit, :can_register, :cgpa)";

    $curr_sem = 1;
    $degree = $_POST['degree'];
    $total_sem = 4;
    $rank = 1;
    if(strcmp($degree, 'Bachelors') == 0) {
	$total_sem = 8;
    }
    $register_limit = 30;
    $can_register = 'No';
    ...
    $statement = $connection->prepare($sql);
    $statement->bindParam(':id', $id, PDO::PARAM_INT);
    $statement->bindParam(':advisor_id', $advisor_id, PDO::PARAM_INT);
    $statement->bindParam(':curr_sem', $curr_sem, PDO::PARAM_INT);
    $statement->bindParam(':total_sem', $total_sem, PDO::PARAM_INT);
    $statement->bindParam(':rank', $rank, PDO::PARAM_INT);
    $statement->bindParam(':register_limit', $register_limit, PDO::PARAM_INT);
    $statement->bindParam(':can_register', $can_register, PDO::PARAM_STR);
    $statement->bindValue(':cgpa', 0.0);
  ...
  <body>
	...
	<li class="nav-item pr-3">
	  <div class="col-sm-12 my-auto">Signup As: </div>
        ...
        <li class="nav-item pr-3">
          <a class="nav-link active" href="Signup_as_Student.php">Student</a>
        </li>
        <li class="nav-item pr-3">
          <a class="nav-link" href="Signup_as_Instructor.php">Instructor</a>
        </li>
	<li class="nav-item pr-3">
    ...
    <div class="dropdown-divider pt-4"></div>
	  ...
	  <fieldset>
	    ...
	    <div id="degreedropdown" class="form-group custom-container2 container">
		...
		<input type="text" class="form-control" name="emergency phone" placeholder="Enter emergency contact number">
            ...
            <div>
              ...
              <a class="btn btn-link" href="Login_as_Admin.php" role="button">Already registered? Login</a>
            </div>
	  </fieldset>
	</form>
      </div>
